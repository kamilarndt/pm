...
metadata:
  hermes:
    tags: [product-management, ai-shipping]
---

# derive-tests — Turn Intent Into Tests

Użycie: `derive-tests` — jeśli nie podano zakresu, działa na całe repozytorium.

The docs say what the system *should* do. An audit finds where the code *doesn't*. Tests are what stop that gap from reopening after the next AI edit. This skill reads the documented intent, turns each load-bearing rule into a concrete test case, sorts them into what to automate, what needs a guarded live run, and what stays manual — then recommends the CI gate that keeps `main` honest.
...
Tests are derived from the docs, so the docs come first. If `/documentation/*.md` is missing or thin, run `document-app` first. (This skill reads `flows.md`, `permissions.md`, and `automation.md` most heavily.) You cannot map coverage to rules you never wrote down — where intent is absent, say so rather than inventing rules to test.
...
- Don't fabricate rules to manufacture coverage. If the docs are silent, the gap is in the docs — run `document-app` first.
