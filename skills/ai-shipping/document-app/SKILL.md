...
Użycie: `document-app` — jeśli nie podano zakresu, dokumentuje całe repozytorium.
...
If a specific scope is provided, document that area. If empty, document the whole repository, prioritizing backend code, auth, data access, background jobs, and anything that sends, schedules, or exposes data.
